...
local disk, MinIO, and real AWS S3.
"""
...
class BronzeStorage:
    """Writes raw GDELT artifacts to the bronze bucket, laid out as::

        s3://<bronze>/<feed>/dt=YYYY-MM-DD/hour=HH/<filename>
    """
    ...
    def key(self, feed: str, partition_path: str, filename: str) -> str:
        return f"s3://{self._bucket}/{feed}/{partition_path}/{filename}"
