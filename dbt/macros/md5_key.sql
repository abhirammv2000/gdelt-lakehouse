{#
  ...
  Dispatches per warehouse so the same models run on every target:
  ...
  the value goes through to_utf8() first. Databricks matches DuckDB and returns hex
  directly, but spells the cast target `string` rather than `varchar`.
#}
{% macro md5_key(expr) %}
...
{% macro databricks__md5_key(expr) %}
